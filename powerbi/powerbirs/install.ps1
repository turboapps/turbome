...
$RegKey = "HKCU:\Software\Microsoft\Microsoft Power BI Desktop SSRS\"
...
$exeConfigPath = "$env:programfiles\Microsoft Power BI Desktop RS\bin\PBIDesktop.exe.config"
...
$mainExe = "$env:PROGRAMFILES\Microsoft Power BI Desktop RS\bin\PBIDesktop.exe"
Write-Host "Version info"
(Get-Item $mainExe).VersionInfo
$version = (Get-Item $mainExe).VersionInfo.FileVersion
"powerbi/powerbirs:$version" | Set-Content "X:\output\image.txt"
